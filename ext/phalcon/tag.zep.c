...
PHP_METHOD(Phalcon_Tag, getDI)
{
	...
	if (Z_TYPE_P(&di) == IS_NULL) {
		ZEPHIR_CALL_CE_STATIC(&di, phalcon_di_di_ce, "getdefault", &_0, 0);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&di);
}
...
PHP_METHOD(Phalcon_Tag, getTitle)
{
	...
	if (prepend) {
		...
		if (!(ZEPHIR_IS_EMPTY(&documentPrependTitle))) {
			ZEPHIR_CALL_FUNCTION(&tmp$$5, "array_reverse", NULL, 305, &documentPrependTitle);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Tag, textArea)
{
	...
	ZEPHIR_CALL_SELF(&code, "renderattributes", NULL, 0, &_1, &params);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_2, "htmlspecialchars", NULL, 295, &content);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_3);
